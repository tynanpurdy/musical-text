			...
			while ((match = sentenceRegex.exec(line)) !== null) {
				...
				if (className && wordCount > 0) {
					...
					const trailingWhitespace =
						sentence.length - sentence.trimEnd().length;
					const contentStart = sentenceStart + leadingWhitespace;
					const contentEnd =
						sentenceStart + sentence.length - trailingWhitespace;
 ...
 */
function detectMarkdownListMarker(
	line: string,
): { content: string; markerLength: number } | null {
	...
	if (orderedMatch) {
		...
		const markerLength =
			indentation.length + number.length + dot.length + spaces.length;
		return {
